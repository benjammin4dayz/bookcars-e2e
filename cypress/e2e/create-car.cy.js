...
describe('Create Car - Form Inputs', () => {
  beforeEach(() => {
    cy.login('backend');
  ...
  it('should select numerous pickup locations', () => {
    const pickupLocations = selectPickupLocations();
...
describe('Create Car - Form Submission', () => {
  beforeEach(() => {
    cy.login('backend');
    cy.visit(BASE_URL + ':3001/create-car');
  });
  ...
  after(() => {
    // clean up the car by deleting it
    cy.wait(5000);
    const deleteButton = ':nth-child(1) > .action > button.MuiButtonBase-root';
